...
class RefreeManagementController extends Controller
{
 ...
 //decline
    ...
    public function refereeProfile(){
        $user = auth()->user()->id;
        $referee =Referee::where('user_id',$user)->first();
        $referee_maincash_hitories = MaincashHitory::where('referee_id', $referee->id)->get();

        // $agent_cash_histories = AgentcashHistory::all()
        ...
        return view('RefereeManagement.referee_profile', compact('referee','agents','results','sum','agentsaleamounts', 'referee_maincash_hitories'));
    }
}
